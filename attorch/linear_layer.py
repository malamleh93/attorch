...
class LinearAutoGrad(torch.autograd.Function):
    ...
    def forward(
        ...
        assert (
            bias is None or weight.shape[1] == bias.shape[0]
        ...
        output = torch.empty(
            (batch_dim, out_feat_dim), device=input.device, dtype=output_dtype
        )
        ...
        grid = lambda META: (
            cdiv(batch_dim, META["BLOCK_SIZE_BATCH"])
    ...
    def backward(
        ...
        with torch.autocast("cuda", dtype=ctx.output_dtype):
            ...
            weight_grad = (
                flattened_input.T @ pre_act_grad if weight.requires_grad else None
            )
