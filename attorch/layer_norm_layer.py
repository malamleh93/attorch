...
class LayerNormAutoGrad(torch.autograd.Function):
    ...
    def forward(
        ...
        output_dtype = get_output_dtype(
            input.dtype, autocast="fp32" if autocast_to_fp32 else None
        )
        ...
        requires_grad = (
            input.requires_grad
            or (scale_by_weight and weight.requires_grad)
            or (add_bias and bias.requires_grad)
        )

        if requires_grad:
            mean = torch.empty(batch_dim, device=input.device, dtype=torch.float32)
            inv_std = torch.empty(batch_dim, device=input.device, dtype=torch.float32)
...
class LayerNorm(nn.LayerNorm):
    ...
    def forward(self, input: Tensor) -> Tensor:
        return LayerNormAutoGrad.apply(
            input, self.weight, self.bias, self.eps, self.autocast_to_fp32
        )
