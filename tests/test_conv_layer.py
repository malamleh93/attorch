...
def test_conv2d_layer(
    ...
    if len(input_shape) == 3 and (
        isinstance(kernel_size, tuple) or isinstance(padding, tuple)
    ):
        return
    ...
    torch.manual_seed(0)
    attorch_conv = getattr(attorch, conv_name)(
        input_shape[1],
        out_dim,
        kernel_size,
        stride=stride,
    ...
    torch.manual_seed(0)
    pytorch_conv = getattr(nn, conv_name)(
        input_shape[1],
        out_dim,
        kernel_size,
        stride=stride,
    ...
    bias_grad_pair = (
        (attorch_conv.bias.grad, pytorch_conv.bias.grad)
        if bias
