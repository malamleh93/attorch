...
def test_batch_norm_layer(
    ...
    attorch_batch_norm = getattr(attorch, bn_name)(
    ...
    residual_grad_pair = (
        (attorch_residual.grad, pytorch_residual.grad) if add_pre_act else (None, None)
    )
    weight_grad_pair = (
        (attorch_batch_norm.weight.grad, pytorch_batch_norm.weight.grad)
        if affine
        else (None, None)
    )
    bias_grad_pair = (
        (attorch_batch_norm.bias.grad, pytorch_batch_norm.bias.grad)
        if affine
        else (None, None)
    )
    assert_close(
