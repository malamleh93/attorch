...
def test_rms_norm_layer(
    ...
    pytorch_rms_norm = nn.RMSNorm(shape[-1], eps, elementwise_affine, device="cuda")
    ...
    weight_grad_pair = (
        (attorch_rms_norm.weight.grad, pytorch_rms_norm.weight.grad)
        if elementwise_affine
        else (None, None)
    )
    assert_close(
        (attorch_input.grad, pytorch_input.grad), weight_grad_pair, rtol=1e-3, atol=1e-3
    )
