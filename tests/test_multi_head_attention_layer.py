...
def test_multi_head_attention_layer(
    ...
    torch.manual_seed(0)
    attorch_multi_head_attention = attorch.MultiheadAttention(
        shape[-1], num_heads, bias=bias, batch_first=True
    )

    torch.manual_seed(0)
    pytorch_multi_head_attention = nn.MultiheadAttention(
        shape[-1], num_heads, bias=bias, batch_first=True, device="cuda"
    )
    ...
    with autocast("cuda", enabled=amp):
        attorch_output = attorch_multi_head_attention(
            attorch_input_q, attorch_input_k, attorch_input_v, causal=causal
        )
        pytorch_output = pytorch_multi_head_attention(
            pytorch_input_q,
            pytorch_input_k,
            pytorch_input_v,
    ...
    assert_close((attorch_output, pytorch_output), rtol=1e-2, atol=1e-2)
    ...
    assert_close(
        (attorch_input_q.grad, pytorch_input_q.grad),
