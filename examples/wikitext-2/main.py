...
def create_dls(
    ...
    dataset = dataset.map(
        lambda input: tokenizer(
            input["text"], max_length=seq_len, padding="max_length", truncation=True
        ),
        batched=True,
    )
...
if __name__ == "__main__":
    parser = ArgumentParser(
        description="Trains and benchmarks a language model on the WikiText-2 dataset."
    )
    parser.add_argument(
        "--model",
        type=str,
    ...
    print("attorch run:")
    main(
        partial(locals()[args.model], use_attorch=True),
        epochs=args.epochs,
    ...
    print("PyTorch run:")
    main(
        partial(locals()[args.model], use_attorch=False),
        epochs=args.epochs,
        batch_size=args.batch_size,
        seq_len=args.seq_len,
        num_workers=args.num_workers,
